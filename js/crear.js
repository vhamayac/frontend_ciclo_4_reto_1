...
/**
 * Validar que campos no estan vacios
 */
 ...
 */
 ...
 const coincidenPassword=()=>
 {
     return (password.value==passwordc.value)?true:false
...
const validar=()=>{
    const val1=validarNombre(nombre.value)?"none":"block"
    const val2=validarEmail(email.value)?"none":"block"
    const val3=validarPassword(password.value)?"none":"block"
    const val4=coincidenPassword()?"none":"block"
    errores[0].style.display=val1
    errores[1].style.display=val2
    errores[2].style.display=val3
    errores[3].style.display=val4
}
